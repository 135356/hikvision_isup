...
//     DATETYPE_EXPECTION = 1,     //出现异常,               ClassType(pOutBuffer) = DWORD *,表示错误码
//     DATETYPE_LINK_INFO,         //设备发起大文件传输连接信息回调；  ClassType(pOutBuffer) = LPNET_ISUP_CMS_LF_LINK_INFO
//     DATETYPE_UPLOAD_RSP,        //文件上传响应结果        ClassType(pOutBuffer) = LPNET_ISUP_CMS_LARGEFILETRANS__UPLOAD_RSP_PARAM
//     DATETYPE_DOWNLOAD,          //文件下载                ClassType(pOutBuffer) = LPNET_ISUP_CMS_LF_DOWNLOAD_PARAM
//     DATETYPE_DOWNLOAD_END,      //文件下载结束            ClassType(pOutBuffer) = HPR_UINT32 *状态值，成功=0，其他值见错误码
...
//     BOOL                bFileFirst;      //第一包数据标记
//     BOOL                bFileEnd;        //最后一包数据标记
//     BYTE                byRes[128];
...
// {
//     DATETYPE_URL = 0,                  //URL                 ClassType(pBuffer) = LPNET_ISUP_CMS_LF_PARAM, buf为URL
//     DATETYPE_INTERACT_BY_BUF,          //按buf传入交互数据        ClassType(pBuffer) = NULL
...
// {
//     char *              pBuf;        //Buf内容参见枚举NET_ISUP_CMS_LF_SET_BUSINESS_PARAM
//     DWORD               dwBufLen;
//     BYTE                byRes[128];
...
// //dwFunType见宏FUN_UPLOAD_SET_PARAM和FUN_DOWNLOAD_SET_PARAM， dwBusinessType参见枚举值NET_ISUP_CMS_LF_SET_BUSINESS_PARAM
// NET_DVR_API BOOL CALLBACK NET_ISUP_CMS_LFSetParam(LONG lLinkHandle, DWORD dwFunType, DWORD dwBusinessType, void* pBuffer);
// //若选择以路径方式将数据传入SDK，lpUploadParam设置为NULL
