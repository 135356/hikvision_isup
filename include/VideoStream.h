...
namespace isup{
    namespace video{
        FILE *video_fp{}; //视频文件句柄
        long preview_handle{}; //预览句柄
        long link_handle{}; //连接句柄
        long lHandle{}; //登出的时候需要
        NET_EHOME_PUSHSTREAM_IN struPushStreamIn = { 0 }; //预览请求推流输入参数，登出时需要session_id
        ...
        int streamF(){
            ...
            struListen.byLinkMode = 0; //0-TCP, 1-UDP
            //开启监听服务
        ...
        void streamOutF(){
            /* 将预览停止请求从 CMS 发送给设备 */
            if(!NET_ECMS_StopGetRealStream(user_id, struPushStreamIn.lSessionID)) {
